...
In the realm of cross-lingual or multilingual natural language processing, it does appear that some languages tend to get more attention than others. Typically well-established languages with a plethora of linguistic words to build from, often thrive while low-resource languages, such as various creoles, are left to languish; as of today, some words within the Saint Lucian Kwéyòl/creole vocabulary, that lack official etymological details. However, the increasing popularity of opinion rich resources such as blogs {% cite kweyol_works_joseph %}, shopping websites, review portals, and social media platforms {% cite kweyol_fb_joseph %} are rapidly attracting business people, governments, and researchers alike. While there are multiple ways to utilize NLP functions, sentiment analysis often garners great interest. It has also been said while there is a lingering 'bias towards contemporary Indo-European languages', treebanks for other language families and treebanks for classical languages are on the rise {% cite nivre2016universal %}. Therefore, it may be helpful to develop a framework to improve the models for linguistics; perhaps an extendable framework that can demonstrate its application to low-resource languages such as creole. 
...
In terms of creole data, I currently have access to public social media data, poems, hymns, and the Saint Lucian national anthem. I also have access to a partially labeled XML dictionary dataset in the target language of Saint Lucian Kwéyòl (but no sentiment data). There are numerical indicators to signify the differences between homonyms, and according to the dictionary's creator
...
- *Need to clean this up. Perhaps reword to use less of the direct quote.*
...
- * Need to add the references mentioned above.*
...
- *I also might want to mention the possible issues with transliteration when dealing with the etymology of words that non-Latin based characters that still somewhat contribute to the creole vocabulary; this mostly would focus on target languages of Tamil and Hindi* {% cite frank2007sources %}.
...
Polysemy is a common occurrence in the low-resource language of Saint Lucian Kwéyòl. An indication of a polysemous verb in English is one that corresponds to different verbs when translated into other languages. For example, one can review the English word for "ask" (for information) and "ask" (for action); this can be interpreted as one word "vragen" in Dutch, however, the majority of other languages use different words for each interpretation in English, like "fragen" and "bitten" in German, and "preguntar" and "pedir" in Spanish. In Saint Lucian Kwéyòl,  this can be seen where the term "mwen" can signify the pronouns for 'I, me, my, or mine'; the term "asou" is a preposition that can mean 'on, on top of, atop, upon', ' off, off of, from', 'toward' or 'about, concerning' {% cite Frank2001kweyol %}.
...
- *Bad format and sourcing for this paragraph. Need to clean this up and add the references mentioned above.*
