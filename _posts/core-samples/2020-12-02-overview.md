...
In terms of creole data, I currently have access to public social media data, poems, hymns, a few documents from the government such as the Saint Lucian national anthem; there are few books but they are mostly physical documents (I need to finish looking into their digital versions). I also have access to a partially labeled XML dictionary dataset in the target language of Saint Lucian Kwéyòl (but no sentiment data). There are numerical indicators to signify the differences between homonyms, and according to the dictionary's creator
